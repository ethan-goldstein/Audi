...
const navMenu = document.getElementById('nav-menu'),
  ...
  const swiperHome = new Swiper('.home__swiper', {
    ...
    pagination: {
      ...
      renderBullet: (index, className) => {
        const carNames = ['A8', 'RS7', 'S8'];
        return `<span class="${className}">${carNames[index] || String(index + 1).padStart(2, '0')}</span>`;
      },
    },
    on: {
      init: function() {
  ...
  swiperHome.on('slideChangeTransitionStart', function() {
    ...
    // Reset animations
    gsap.set([image, titles, specs, button], { opacity: 0, y: 30 });
    
    // Animate in new slide
    const slideTl = gsap.timeline();
    slideTl
      .to(image, { x: 0, opacity: 1, duration: 0.8, ease: 'power3.out' })
      .to(titles, { y: 0, opacity: 1, duration: 0.6, ease: 'power2.out' }, '-=0.4')
      .to(specs, { y: 0, opacity: 1, duration: 0.5, ease: 'power2.out' }, '-=0.3')
      .to(button, { y: 0, opacity: 1, duration: 0.5, ease: 'power2.out' }, '-=0.3');
  });
